...
@Component({
  selector: 'app-gs-tallas',
  templateUrl: './gs-tallas.component.html',
  styleUrls: ['./gs-tallas.component.scss']
})
export class GsTallasComponent implements OnInit {
    ...
    try {
      if (event.newData) {
        if (event.newData["descripcion"].trim().length > 0) {
          this.conectorApi.Post('tallas/registro', event.newData).subscribe(
            (data) => {
  ...
  onActualizar(event): void {
    try {
      if (event.newData) {
        if (event.newData["descripcion"].trim().length > 0) {
          this.conectorApi.Patch(`tallas/actualizar/${event.data["id"]}`, event.newData).subscribe(
            (data) => {
              let apiResult = data as ApiRest;
              if (apiResult.codigo == 0) {
                this.toastrService.success(apiResult.respuesta, 'Información!');
      ...
      if (result.value) {
        ...
        try {
          if (event.data) {
            ...
            this.conectorApi.Patch(`tallas/actualizar/${event.data["id"]}`, event.data).subscribe(
              (data) => {
                let apiResult = data as ApiRest;
                if (apiResult.codigo == 0) {
