...
@Component({
  selector: 'app-gs-estado-pedido',
  templateUrl: './gs-estado-pedido.component.html',
  styleUrls: ['./gs-estado-pedido.component.scss']
})
export class GsEstadoPedidoComponent implements OnInit {
      ...
      if (result.value) {
        ...
        try {
          if (event.data) {
            ...
            this.conectorApi.Patch(`estadopedido/actualizar/${event.data["id"]}`, event.data).subscribe(
              (data) => {
                let apiResult = data as ApiRest;
                if (apiResult.codigo == 0) {
