...
import { FirebaseService } from '../../../shared/service/firebase/todo.service';
...
export class ToDoFirebaseComponent implements OnInit {
  ...
  constructor(private toastrService: ToastrService, private firebaseService: FirebaseService, private router: Router, private route: ActivatedRoute) { }
  ...
  public addTask(text) {
    ...
    this.firebaseService.createTask(text)
      .then(
        res => {
          this.resetFields();
          this.router.navigate(['/to-do/to-do-firebase']);
        }
      )
